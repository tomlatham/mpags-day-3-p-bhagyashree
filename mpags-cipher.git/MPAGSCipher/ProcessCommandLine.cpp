...
bool processCommandLine(const std::vector<std::string>& args, ProgramSettings& cmd_settings)
{
  ...
  for (size_type i {1}; i < nArgs; ++i) {
    ...
    if (args[i] == "-h" || args[i] == "--help") {
      ...
      cmd_settings.helpRequested = true;
      break;
    }
    else if (args[i] == "--version") {
      // Set the indicator and terminate the loop
      cmd_settings.versionRequested = true;
      break;
    }
    else if (args[i] == "-i") {
      ...
      if (i == nArgs-1) {
        ...
        cmd_settings.inputFile = args[i+1];
        ++i;
      }
    }
    else if (args[i] == "-o") {
      ...
      if (i == nArgs-1) {
        ...
        // Got filename, so assign value and advance past it
        cmd_settings.outputFile = args[i+1];
        ++i;
      }
    }
    else if (args[i] == "-k") {
      ...
      if (i == nArgs-1) {
        ...
        cmd_settings.cipher_key = args[i+1];
        ++i;
      }
    }
    else if ( args[i] == "--encrypt" ) {
            cmd_settings.cipher_mode = CipherMode::Encrypt;
    }
    else if ( args[i] == "--decrypt" ) {
            cmd_settings.cipher_mode = CipherMode::Decrypt;
    }
    else {
